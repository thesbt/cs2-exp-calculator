...
function calculateLevel() {
    ...
    if (selectedMode === "deathmatch") {
        ...
        result.innerHTML = `You can level up by getting <nobr class="dynamic-content"> ${requiredPoints}</nobr>  score in Deathmatch / Arms Race game mode.`;
    } else if (selectedMode === "casual") {
        requiredPoints = Math.ceil(calculateCasualXP(experience, selectedBoost));
        result.innerHTML = `You can level up by having <nobr class="dynamic-content"> ${requiredPoints}</nobr> points in Casual game mode.`;
    } else if (selectedMode === "competitive") {
        requiredPoints = calculateCompetitiveXP(experience, selectedBoost);
        result.innerHTML = `You can level up by winning <nobr class="dynamic-content"> ${requiredPoints}</nobr> rounds in Competitive / Premier game mode.`;
    } else if (selectedMode === "wingman") {
        requiredPoints = Math.ceil(calculateWingmanXP(experience, selectedBoost));
        result.innerHTML = `You can level up by winning <nobr class="dynamic-content"> ${requiredPoints}</nobr> rounds in Wingman game mode.`;
    }
